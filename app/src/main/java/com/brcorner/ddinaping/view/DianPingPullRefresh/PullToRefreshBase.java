	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_MOVE: {
                ...
                if (distance >= 0) {
                    headerLayout.doSetProgress(distance);
                } else {
                    headerLayout.doSetProgress(0);
                }
				if (isBeingDragged) {
...
//		if (null != headerLayout) {
//			headerLayout.setTextColor(context.getResources().getColor(R.color.tl_pull_bg));
//		}
		// Styleables from XML
		...
		if (a.hasValue(R.styleable.PullToRefresh_adapterViewBackground)) {
			refreshableView.setBackgroundResource(a.getResourceId(R.styleable.PullToRefresh_adapterViewBackground,
					context.getResources().getColor(R.color.tl_pull_bg)));
		}
		a.recycle();
